...
if __name__ == '__main__':
    ...
    for generation in range(MAX_GENERATIONS):
        ...
        m_fitness_scores = []
        for individual in population:
            mutated_ind, mflag = mutation_rand(individual, MUTATION_RATE, GENE_LENGTH)
            if mflag == 1:
    ...
    for i in range(len(best_individual_over_generations)):
        be = fs_fun(best_individual_over_generations[i], ecYeast, ecFSEOF_tabel, GeneProteinMap, 'r_1589',
                    wildSolution_moma, metabolic_solution)
        print('***********', be)
    ...
    # 将DataFrame保存到CSV文件中
    df.to_csv('/Users/liaowenbin/Desktop/GA_checking.csv', index=False)
